  ...
  $w("#userTable").onRowSelect(async (event) => {
    ...
    try {
      ...
      if (userInfo) {
        ...
        $w(
          "#text10"
        ).text = `选中的用户积分为: ${points}，查询的这个用户是${userInfo.name}，slug为：${userInfo.userslug}`; // 显示积分信息和用户信息
  ...
  $w("#commentTable").onRowSelect(async (event) => {
    ...
    try {
      ...
      if (userInfo) {
        ...
        $w(
          "#text10"
        ).text = `选中用户一共评论了${uniqueWorkCount}个作品，查询的用户是${userInfo.name}，slug是${userInfo.userslug}`; // 显示评论数量和用户信息
        $w("#inputId").value = userId; // 填入userId
      } else {
        $w("#text10").text = `找不到用户ID为: ${userId} 的用户信息`; // 用户信息未找到的情况
      }
...
async function updatePoints() {
  ...
  wixWindow
    .openLightbox("PointsConfirmation", { userId, pointsToAdd })
    .then((data) => {
