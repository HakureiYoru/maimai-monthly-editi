...
import {
  getUserPublicInfo,
  getUserInfoBySlug,
} from "backend/getUserPublicInfo.jsw";
// @ts-ignore
...
export async function enterContest034_beforeInsert(item, context) {
  try {
    const maxSequenceId = await getMaxSequenceId(
...
export function enterContest034_afterInsert(item, context) {
  ...
  logInfo(
    ...
    `新作品创建，准备上传到Majnet: ${item.firstName || "未命名"}`
  );
      ...
      .then(async (result) => {
        if (result.success) {
          ...
          try {
            ...
            logError(
              "enterContest034_afterInsert - 更新上传状态失败",
              updateError,
              {
                itemId: item._id,
...
export function jobApplication089_afterInsert(item, context) {
  ...
  logInfo(
    ...
    `用户报名成功，正在检查是否为Qualified用户: ${item._owner}`
  );
    ...
    checkAndMarkQualifiedUser(item)
      .then(async (result) => {
        if (result.isQualified) {
          logInfo(
            "jobApplication089_afterInsert",
            `用户 ${item._owner} 已自动标记为Qualified用户`
          );
          ...
          try {
            // 先获取当前完整数据
            const currentItem = await wixData.get(
              COLLECTIONS.JOB_APPLICATION_089,
              item._id
            );
            ...
            logInfo(
              "jobApplication089_afterInsert",
              `已标记用户 ${item._owner} 为Qualified用户`
            );
          } catch (updateError) {
            logError(
              "jobApplication089_afterInsert - 更新Qualified状态失败",
              updateError,
              {
                itemId: item._id,
                userId: item._owner,
      ...
      .catch((error) => {
        logError(
          "jobApplication089_afterInsert - 检查Qualified状态异常",
          error,
          {
            itemId: item._id,
            userId: item._owner,
...
async function checkAndMarkQualifiedUser(applicationItem) {
  try {
    ...
    logError("checkAndMarkQualifiedUser", error, {
      userId: applicationItem._owner,
    });
    return { isQualified: false };
  }
}
