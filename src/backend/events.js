...
export function wixForum_onPostCreated(event) {
    ...
    // 发帖奖励积分
    if (postOwnerId) {
        updateUserPoints(postOwnerId, POINTS_CONFIG.POST_CREATE, false, true)
            .catch((error) => {
                logError('wixForum_onPostCreated - updateUserPoints', error, { postId, postOwnerId });
            });
    }
}
...
export function wixForum_onCommentCreated(event) {
    ...
    if (commentOwnerId) {
        updateUserPoints(commentOwnerId, POINTS_CONFIG.COMMENT_CREATE, false, true)
            .catch((error) => {
                logError('wixForum_onCommentCreated - updateUserPoints', error, { commentId, commentOwnerId });
            });
    }
}
...
export async function wixForum_onCommentMarkedAsBest(event) {
    ...
    try {
        const commentResult = await wixData.query(COLLECTIONS.FORUM_COMMENTS).eq('_id', bestCommentId).find();
        if (commentResult.items.length > 0) {
            const bestAnswerOwnerId = commentResult.items[0]._ownerId;
            logEvent(EVENT_TYPES.COMMENT_MARKED_AS_BEST, { bestCommentId }, bestAnswerOwnerId);
            
            updateUserPoints(bestAnswerOwnerId, POINTS_CONFIG.BEST_ANSWER, false, false)
                .catch((error) => {
                    logError('wixForum_onCommentMarkedAsBest - updateUserPoints', error, { bestCommentId, bestAnswerOwnerId });
                });
        } else {
...
export async function wixForum_onCommentUnmarkedAsBest(event) {
    ...
    try {
        ...
        if (commentResult.items.length > 0) {
            const bestAnswerOwnerId = commentResult.items[0]._ownerId;
            logEvent(EVENT_TYPES.COMMENT_UNMARKED_AS_BEST, { bestCommentId }, bestAnswerOwnerId);
            
            updateUserPoints(bestAnswerOwnerId, POINTS_CONFIG.BEST_ANSWER_REMOVE, false, false)
                .catch((error) => {
                    logError('wixForum_onCommentUnmarkedAsBest - updateUserPoints', error, { bestCommentId, bestAnswerOwnerId });
                });
        } else {
            console.error('Comment not found:', bestCommentId);
        }
...
export async function wixForum_onCommentLiked(event) {
    ...
    try {
        const comment = await wixData.query(COLLECTIONS.FORUM_COMMENTS).eq('_id', likedCommentId).find();
...
export async function wixForum_onCommentUnliked(event) {
    ...
    try {
        const comment = await wixData.query(COLLECTIONS.FORUM_COMMENTS).eq('_id', unlikedCommentId).find();
        if (comment.items.length > 0) {
            const authorId = comment.items[0]._ownerId;
...
export async function wixForum_onPostLiked(event) {
    ...
    try {
        const post = await wixData.query(COLLECTIONS.FORUM_POSTS).eq('_id', likedPostId).find();
...
export async function wixForum_onPostUnliked(event) {
    ...
    try {
        const post = await wixData.query(COLLECTIONS.FORUM_POSTS).eq('_id', unlikedPostId).find();
        if (post.items.length > 0) {
            const authorId = post.items[0]._ownerId;
...
export async function wixForum_onCommentUpvoted(event) {
    ...
    try {
        const comment = await wixData.query(COLLECTIONS.FORUM_COMMENTS).eq('_id', upvotedCommentId).find();
        if (comment.items.length > 0) {
            const authorId = comment.items[0]._ownerId;
...
export async function wixForum_onCommentUnvoted(event) {
    ...
    try {
        const comment = await wixData.query(COLLECTIONS.FORUM_COMMENTS).eq('_id', unvotedCommentId).find();
        if (comment.items.length > 0) {
            const authorId = comment.items[0]._ownerId;
            logEvent(EVENT_TYPES.COMMENT_UNVOTED, { unvotedCommentId }, authorId);
            await updateUserPoints(authorId, POINTS_CONFIG.UPVOTE_REMOVE, false, false);
        }
    } catch (error) {
...
export async function wixForum_onCommentDeleted(event) {
    ...
    try {
        const commentQuery = wixData.query(COLLECTIONS.FORUM_COMMENTS).eq('_id', deletedCommentId);
        const results = await commentQuery.find();
        
        if (results.items.length > 0) {
            const comment = results.items[0];
            const authorId = comment._ownerId;
            await updateUserPoints(authorId, POINTS_CONFIG.COMMENT_DELETE, false, false);
        }
    } catch (error) {
...
async function logEvent(eventType, eventDetails, userId) {
    ...
    wixData.insert(COLLECTIONS.EVENT_LOGS, logData)
        .catch((error) => {
...
async function logPostCreatedEvent(postId, postTitle, postOwnerId, postPageUrl) {
    ...
    wixData.insert(COLLECTIONS.POST_LOGS, logData)
        .catch((error) => {
